...
async def test_amazon_s3_driver_upload_file__success(
    ...
    mock_put_object = (
        mock_session.create_client.return_value.__aenter__.return_value.put_object
    )
...
async def test_amazon_s3_driver_upload_file__error_status_code(
    ...
    assert "It was not possible to upload the file on S3" == str(
        exception.value
    )
...
async def test_amazon_s3_driver_delete_file__success(
    ...
    mock_get_session.return_value = mock_session
    mock_delete_object = (
        mock_session.create_client.return_value.__aenter__.return_value.delete_object
    )
    ...
    mock_delete_object.assert_called_once_with(
        Bucket=bucket, Key=example_file_path
    )
...
async def test_amazon_s3_driver_delete_file__status_code_error(
    ...
    assert "It was not possible to delete the file on S3" == str(
        exception.value
    )
